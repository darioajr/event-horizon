...
class ProduceResult:
    ...
    target: str  # "kafka" ou "eventhorizon"
...
def print_comparison(kafka_result: ProduceResult, eh_result: ProduceResult):
    """Exibe comparação lado a lado"""
    table = Table(title="Comparação: Kafka vs Event Horizon", show_header=True, header_style="bold magenta")
    
    table.add_column("Métrica", style="dim")
    table.add_column("Kafka", justify="right")
    table.add_column("Event Horizon", justify="right")
    ...
    table.add_row(
        ...
        f"{eh_result.messages_per_second:,.0f}",
        diff(kafka_result.messages_per_second, eh_result.messages_per_second)
    )
    table.add_row(
        "  MB/s",
        f"{kafka_result.mb_per_second:.2f}",
        f"{eh_result.mb_per_second:.2f}",
        diff(kafka_result.mb_per_second, eh_result.mb_per_second)
    )
    table.add_row("", "", "", "")
    table.add_row("[bold]Latência (ms)[/bold]", "", "", "")
    table.add_row(
        ...
        f"{eh_result.latency_p50:.3f}",
        diff(kafka_result.latency_p50, eh_result.latency_p50, higher_is_better=False)
    )
    table.add_row(
        "  P99",
        f"{kafka_result.latency_p99:.3f}",
        f"{eh_result.latency_p99:.3f}",
        diff(kafka_result.latency_p99, eh_result.latency_p99, higher_is_better=False)
    )
    table.add_row(
        "  P99.9",
        f"{kafka_result.latency_p999:.3f}",
        f"{eh_result.latency_p999:.3f}",
        diff(kafka_result.latency_p999, eh_result.latency_p999, higher_is_better=False)
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark de produção: Event Horizon vs Apache Kafka",
...
Exemplos:
  ...
  python produce_benchmark.py --compare --messages 1000000 --message-size 1024
    ...
    mode.add_argument("--eventhorizon-only", action="store_true", help="Testar apenas Event Horizon")
    mode.add_argument("--compare", action="store_true", help="Comparar Kafka e Event Horizon")
    
    # Event Horizon
    parser.add_argument("--eh-host", default="localhost", help="Host do Event Horizon (default: localhost)")
    parser.add_argument("--eh-port", type=int, default=9092, help="Porta do Event Horizon (default: 9092)")
    ...
    console.print("\n[bold blue]═══════════════════════════════════════════════════════════[/bold blue]")
    console.print("[bold blue]  Benchmark de Produção: Event Horizon vs Apache Kafka[/bold blue]")
    ...
    # Testar Event Horizon
    if args.eventhorizon_only or args.compare:
        console.print("\n[bold cyan]━━━ Event Horizon ━━━[/bold cyan]")
        
        bootstrap_servers = f"{args.eh_host}:{args.eh_port}"
        console.print(f"[dim]Conectando a: {bootstrap_servers}[/dim]")
        
        try:
            eh_result = run_produce_benchmark(bootstrap_servers, config, "Event Horizon")
            results.append(eh_result)
            print_result(eh_result)
        except Exception as e:
            console.print(f"[red]✗[/red] Erro ao conectar ao Event Horizon: {e}")
            console.print("[yellow]Certifique-se de que o Event Horizon está rodando.[/yellow]")
            sys.exit(1)
    
    # Comparação
    if args.compare and kafka_result and eh_result:
        console.print("\n")
        print_comparison(kafka_result, eh_result)
