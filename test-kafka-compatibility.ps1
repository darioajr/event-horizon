...
# Script para testar compatibilidade binária do eventhorizon com Kafka
# Descobre automaticamente tópicos e partições a partir da estrutura de diretórios
#
# Uso: .\test-kafka-compatibility.ps1
...
if ($Help) {
    Write-Host @"
eventhorizon Kafka Binary Compatibility Test
===========================================

Este script importa automaticamente os dados do eventhorizon para o Kafka,
...
Uso:
    .\test-kafka-compatibility.ps1           # Executar importacao
    .\test-kafka-compatibility.ps1 -Clean    # Limpar ambiente
    .\test-kafka-compatibility.ps1 -Help     # Mostrar ajuda

Apos a execucao, acesse http://localhost:8080 para visualizar no Kafka UI.
"@
    exit 0
}
...
Write-Host "  eventhorizon -> Kafka Import Tool" -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
Write-Host ""
...
foreach ($topic in $topics.Keys | Sort-Object) {
    foreach ($p in $topics[$topic]) {
        $srcLog = "/eventhorizon-data/$($p.Dir)/00000000000000000000.log"
...
Write-Host ""
Write-Host "Iniciando Kafka com dados do eventhorizon..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "Para limpar: .\test-kafka-compatibility.ps1 -Clean" -ForegroundColor Gray
